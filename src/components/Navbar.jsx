...
function Navbar({ toggleSidebar, isSidebarOpen }) {
  ...
  return (
    <header className="bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-800 h-16 flex items-center justify-between px-4 md:px-8 sticky top-0 z-10 flex-shrink-0 shadow-sm transition-colors duration-200 relative">
      ...
      <AnimatePresence>
        {mobileSearchOpen && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute inset-0 bg-white dark:bg-slate-900 flex items-center px-4 z-20"
          >
            <button onClick={() => setMobileSearchOpen(false)} className="p-2 mr-2 text-gray-500 dark:text-slate-400 rounded-lg">
            ...
            <div className="relative flex-1" ref={searchRef}>
              ...
              <input
                ...
                className="block w-full pl-10 pr-3 py-2 border border-gray-200 dark:border-slate-700 rounded-lg text-base placeholder-gray-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-gray-50 dark:bg-slate-800 text-gray-900 dark:text-white transition-all min-h-[44px]"
                placeholder="Search..."
              />
      ...
      <div className="flex items-center gap-2 md:gap-4 flex-1">
        <button 
          ...
          className="text-gray-400 hover:text-gray-600 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors p-2 rounded-lg"
        >
          <svg className="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
        </button>
        
        {/* Mobile Search Icon */}
        <button 
          onClick={() => { setMobileSearchOpen(true); setTimeout(() => inputRef.current?.focus(), 100); }}
          className="md:hidden p-2 text-gray-400 hover:text-gray-600 dark:text-slate-400 dark:hover:text-slate-200 rounded-lg"
        >
