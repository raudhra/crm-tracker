      ...
      <AnimatePresence>
        {isHovered && (
          <motion.div
            ...
            className="absolute left-full ml-4 px-2.5 py-1.5 bg-gray-900 text-white text-xs font-medium rounded-lg whitespace-nowrap z-50 pointer-events-none shadow-lg"
          >
            {text}
...
function Sidebar({ isSidebarOpen }) {
  ...
  return (
    <motion.div 
      ...
      className="min-h-screen bg-white border-r border-gray-100 flex flex-col relative z-20"
    >
      <div className={`p-6 flex items-center ${isSidebarOpen ? 'gap-3' : 'justify-center'} overflow-hidden whitespace-nowrap`}>
        <div className="bg-indigo-600 text-white w-8 h-8 rounded-lg flex items-center justify-center font-bold text-lg shadow-sm shadow-indigo-200 flex-shrink-0">C</div>
        {isSidebarOpen && <span className="font-bold text-xl text-gray-800 tracking-tight">ClientFlow</span>}
      </div>
      ...
      <div className="p-4 mt-auto mb-4" ref={profileRef}>
        <div className="relative">
          <button 
            ...
            className={`w-full flex items-center ${isSidebarOpen ? 'gap-3 px-3 py-2 justify-between' : 'justify-center p-2'} bg-gray-50 rounded-xl border border-gray-100 hover:bg-gray-100 transition-colors duration-200`}
          >
            <div className="flex items-center gap-3">
              {user?.avatar ? (
                <img src={user.avatar} alt={user.name} className="w-9 h-9 rounded-full object-cover flex-shrink-0" />
              ) : (
                <div className="w-9 h-9 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-sm shadow-sm flex-shrink-0">
                  {user?.name?.[0] ?? 'U'}
                </div>
              )}
              {isSidebarOpen && (
            ...
            {isSidebarOpen && (
              <svg className="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 15l7-7 7 7"></path></svg>
            )}
          </button>
          ...
          <AnimatePresence>
            {profileMenuOpen && (
              <motion.div
                ...
                className={`absolute bottom-full mb-2 ${isSidebarOpen ? 'left-0 right-0' : 'left-0 w-48'} bg-white border border-gray-100 rounded-xl shadow-xl overflow-hidden py-1 z-50`}
              >
                <button
